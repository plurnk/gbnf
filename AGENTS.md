...
- **`build:fetchLlamaGrammar` → `scripts/fetchLlamaGrammar.sh`** copies llama.cpp's GBNF
  source into `llama/` **verbatim, from a pinned upstream commit, never confabulated**
  (Charter §6). It records provenance (upstream repo URL + commit SHA + copied file list) so
  any divergence we later introduce while adapting toward `llama-gbnf.c` is auditable.
...
- **The agent owns all git and GitHub ceremony.** Branch, stage, commit, push, open and
  merge PRs autonomously via `gh` — **do not ask the user to confirm routine git/GitHub
  steps.** Surface a git matter only when it is genuinely necessary: an irreversible action
  on shared history (force-push, hard reset, branch delete, rebase of pushed commits) where
  intent is ambiguous, or a conflict/decision only the user can resolve. Routine ceremony is
